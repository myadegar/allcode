...
def load_model():
    model_path = r"D:\TextMining\SemanticSearch\word2vector\model\paragraph_7_cbow_w2v.mod"
...
def main():
    ...
    if st.button("Search!"):
        ...
        positive_query = [normalizer.normalize(query).replace(' ', '_') for query in positive_query]
        negative_query = [normalizer.normalize(query).replace(' ', '_') for query in negative_query]
...
if __name__ == "__main__":
    main()
    # streamlit run run_semantic_search.py